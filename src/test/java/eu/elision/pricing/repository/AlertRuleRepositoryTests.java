...
@ActiveProfiles("test")
@SpringBootTest
class AlertRuleRepositoryTests {
...
//        alertRule2 = alertRuleRepository.save(alertRule2);
        ...
        this.alertRules =
            List.of(alertSettings.getAlertRules().get(0), alertSettings.getAlertRules().get(1));
    ...
    @Test
    void countRulesCorrectly() {
        User user = userRepository.findAll().get(0);

        assertEquals(2, alertRuleRepository.count());

        assertEquals(2,
            alertRuleRepository.countAllByAlertSettings_IdAndIdIn(user.getAlertSettings().getId(),
                this.alertRules.stream().map(AlertRule::getId).collect(
                    Collectors.toList())));

        assertEquals(1,
            alertRuleRepository.countAllByAlertSettings_IdAndIdIn(user.getAlertSettings().getId(),
                List.of(this.alertRules.get(0).getId())));
