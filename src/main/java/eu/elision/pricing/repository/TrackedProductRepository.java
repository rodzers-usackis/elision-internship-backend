...
public interface TrackedProductRepository extends JpaRepository <TrackedProduct, UUID> {
    ...
    @Transactional
    @Modifying
    @Query("DELETE FROM TrackedProduct t WHERE t.clientCompany.id = :companyId AND t.id IN :trackedProductIds")
    void deleteTrackedProducts(@Param("companyId") UUID companyId, @Param("trackedProductIds") List<UUID> trackedProductIds);
}
